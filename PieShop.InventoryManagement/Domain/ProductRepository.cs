...
{
    private string directory = @"Domain/Data/";
    private string productsFileName = "products.txt";

    private void CheckForExistingProductFile()
    {
        string path = Path.Combine(directory, productsFileName);
...
public List<Product> LoadProductsFromFile()
{
    ...
    try
    {
        ...
        for (int i = 0; i < productsAsString.Length; i++)
        {
            ...
            if (!success)
            {
                amountInStock = 10;//default value
            }
            Price price = new Price(itemPrice, currency);
            Product product = new Product(productId, name, description, unitType, amountInStock, maxItemsInStock, price);
